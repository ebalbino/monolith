...
fn update_button(button: &mut Button, down: bool) {
    let was_down = button.down;
    button.down = down;
    button.repeat = down && was_down;
    button.pressed = !was_down && button.down;
    button.released = was_down && !button.down;
}

fn update_analog_button(button: &mut AnalogButton, value: f32) {
    let signal = button.signal.get_mut();
    let inner = button.inner.get_mut();

    signal.value = value;
    update_button(inner, value >= signal.threshold);
}
